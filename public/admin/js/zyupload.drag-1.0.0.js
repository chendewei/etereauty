	...
	//					edit             : true,  						      // 是否可以编辑文件（裁剪）
				...
				this.createHtml = function(){
					...
					if(para.dragDrop){
						...
						html += '				<div id="status_info" class="infoImg">选中0张文件，共0B</div>';
						html += '				<div class="btns">';
						...
						html += '							<img class="upload_image" src="zyupload/skins/images/add_img.png"/>';
						html += '						</div>'; 
						html += '					</a>';
						html += '				</div>';
						html += '			</div>';
				...
				this.funDisposePreviewHtml = function(file, e){
					...
					if (file.type.indexOf("image") == 0) {
						...
						html += '	<p id="uploadSuccess_'+file.index+'" class="file_success">上传成功</p>';
						html += '	<p id="uploadError_'+file.index+'" class="file_error">上传失败，请重试</p>';
						html += '</div>';
